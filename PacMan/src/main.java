...
public class main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        final double START_X = 250.0, START_Y = 480.0;
        ...
        // Create ghoul
        Circle charlemagne = new Circle(250, 150, RADIUS);
        charlemagne.setFill(Color.RED);
        charlemagne.setStroke(Color.RED);
        double charlemagneVelocity = 0.60;


        Pane pane = new Pane(pak, charlemagne);
        pane.setStyle("-fx-background-color: black; -fx-border-color: blue; -fx-border-width: 10px; -fx-border-radius: 6px;");
        ...
        gameScene.setOnKeyPressed(event -> {
            ...
            // if pak touches the ghoul, end game
            if (pak.getBoundsInParent().intersects(charlemagne.getBoundsInParent())) {
                death.setDeath();
                primaryStage.setScene(death.deathScene);
                primaryStage.setFullScreen(true);
                game.mediaPlayer.stop();
        ...
        AnimationTimer charlemagneTimer = new AnimationTimer() {
            double charlemagneX = charlemagne.getCenterX();
            double charlemagneY = charlemagne.getCenterY();

            @Override
            public void handle(long now) {
                // Get the distance between Pak and Charlemagne
                double deltaX = pak.getCenterX() - charlemagneX;
                double deltaY = pak.getCenterY() - charlemagneY;
                double distance = Math.sqrt(Math.pow(deltaX, 2) + Math.pow(deltaY, 2));

                // Move Charlemagne towards Pak
                if (distance > 0) {
                    charlemagneX += charlemagneVelocity * deltaX / distance;
                    charlemagneY += charlemagneVelocity * deltaY / distance;
                }

                // Update the position of Charlemagne
                charlemagne.setCenterX(charlemagneX);
                charlemagne.setCenterY(charlemagneY);
            }
        };
        // Start Charlemagne's animation
        charlemagneTimer.start();
    }
}
